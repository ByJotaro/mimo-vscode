...
export class SidebarProvider implements vscode.WebviewViewProvider {
      ...
      switch (msg.type) {
        ...
        case 'openFilePath':
          if (typeof msg.path === 'string' && msg.path.trim()) {
            try {
              const p = msg.path.trim().replace(/^file:\/\//, '');
              const uri = vscode.Uri.file(p);
              await vscode.window.showTextDocument(uri, { preview: true });
            } catch (e) {
              this.log.appendLine('[openFile] ' + String(e).slice(0, 120));
            }
          }
          break;
        case 'abort':
  ...
  private onCliEvent(ev: {
    ...
    if (ev.type === 'sessionTitle') {
      const sid = (ev as any).sessionId;
      const title = String((ev as any).title || '').trim();
      if (title && (!sid || sid === this.currentSessionId)) {
        this.post({ type: 'sessionTitle', sessionId: sid || this.currentSessionId, title });
      }
    }
    if (ev.type === 'usage') {
      this.post({
...
<body>
  ...
  <footer class="input-area">
    <div class="input-row">
      <textarea id="prompt" rows="2" placeholder="Message MiMo…  Enter send · Shift+Enter newline · / for commands"></textarea>
